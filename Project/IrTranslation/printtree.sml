(* This file contains function to print an IR tree *)

signature PRINTTREE = sig
     sig val printTree : TextIO.outstream * Tree.stm -> unit 
end

structure PrintTree : PRINTTREE = struct
    structure T = Tree
    ...
    |   stm (T.MOVE (a, b), d) = ( indent d; sayln "MOVE("; exp (a, d + 1); sayln ","; exp (b, d + 1); say ")" )
    |   stm (T.EXP e, d) = ( indent d; sayln "EXP("; exp (e, d + 1); say ")" )

    and exp (T.BINOP (p, a, b), d) = ( indent d; say "BINOP("; binop p; sayln ","; exp (a, d + 1);
                                       sayln ","; exp (b, d + 1); say ")" )
    |   exp (T.MEM e, d) = ( indent d; sayln "MEM("; exp (e, d + 1); say ")" )
    |   exp (T.TEMP t, d) = ( indent d; say "TEMP t"; say (Int.toString t) )
    |   exp (T.ESEQ (s, e), d) = ( indent d; sayln "ESEQ("; stm (s, d + 1); sayln ","; exp (e, d + 1); say ")" )
    |   exp (T.NAME lab, d) = ( indent d; say "NAME "; say (Symbol.name lab) )
    |   exp (T.CONST i, d) = ( indent d; say "CONST "; say (Int.toString i) )
    |   exp (T.CALL (e, el), d) = ( indent d; sayln "CALL("; exp(e,d+1); app (fn a => (sayln ","; exp (a, d + 2))) el; say ")" )
    ...
    fun printTree (outstream, s0) = ( stm (s0, 0); sayln ""; TextIO.flushOut outstream )
end
