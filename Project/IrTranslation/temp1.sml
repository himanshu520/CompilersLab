...
structure Semant = struct 
    ...
    |   transDec (venv, tenv, A.TypeDec lst) = let fun checkTypRep ((x::xs) : { name : S.symbol, ty : T.ty, pos : A.pos }, lst) = if List.find (fn t => t = x) lst then (true, #pos x) else checkTypRep (xs, (#name x) :: lst)
                                                   |   checkTypRep (_, _) = (false, 0)
                                                   val checkTypRepVal = checkTypRep (lst, [])
                                               in if #1 checkTypRepVal then ( error (#2 checkFunRepVal) "type already declared"; { venv = venv, tenv = tenv} )
                                               else let fun addTypeName ({ name, ty, pos }, env) = S.enter (env, name, T.NAME (name, ref NONE))
                                                    val tenv' = foldl addTypeName tenv lst
                                                    fun completeTypeName ({ name, ty, pos }, env) = (print ("Type for " ^ S.name name ^ " is " ^ (case transTy (tenv', ty) of (T.NAME (p, ref NONE)) => S.name p ^ " none\n" | T.NAME (p, _)=> S.name p ^ "\n"| _=> "\n"));S.enter (env, name, transTy (tenv', ty)))
                                                    val tenv'' = foldl completeTypeName tenv lst
                                                    fun validateTy ({ name, ty, pos }, cnt) = ( print ("validating " ^ S.name name ^ "\n");case S.look (tenv'', name) of NONE => ( error pos ("type '" ^ S.name name ^ "' not valid"); false )
                                                                                                                            | SOME (T.NAME (nm, tyref)) => if nm = name andalso cnt = 0 then (print ("false from here for " ^ S.name name ^ " : " ^ S.name nm);false)
                                                                                                                                                           else validateTy ({ name = nm, ty = ty, pos = pos }, 0)
                                                                                                                           | _ => true )
                                                   fun checkCyclicTyDec (x :: xs) = if validateTy (x, 1) then (print "returned true\n";checkCyclicTyDec xs) else (print "returned false\n";(true, #pos x))
