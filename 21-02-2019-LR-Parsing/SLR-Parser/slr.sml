...
(* Function to fill the SLR table with shift, accept and goto actions *)
...
(* Function to fill the SLR table with reduce actions *)
...
(* Function to print the SLR table *)
fun printLrTable stNum = if stNum >= !(StateMap.cnt) then ()
                         else (printState stNum; printLrTable (stNum + 1));

(* Using the functions above to create and print the SLR parsing table *)
StateMap.getProxy (closure (!initialStateSlr));
