...
structure Semant = struct 
    ...
    and fun transDec (venv, tenv, A.FunctionDec ) = { venv = venv, tenv = tenv }
    |   fun transDec (venv, tenv, A.VarDec { name, escape, typ, init, pos }) = ( case typ of SOME (sym, pos) => ( let val typty = S.look (tenv, typ)
                                                                                                                      val initty = transExp (venv, tenv) init
                                                                                                                  in ( case typety of SOME t => if (actual_ty t) <> initty then error pos "type '" ^ S.name typ ^ "' not matches the initialisation expression for variable '" ^ S.name name "'" else ()
                                                                                                                                     | _ => error pos "type '" S.name typ "' is not defined" )
                                                                                                                  end;
                                                                                                                  { tenv = tenv, venv = S.enter (venv, name, E.VarType typty) } )
                                                                                           | _ => transDec (tenv, venv, A.VarDec { name = name, escape = escape, typ = #ty (transExp (tenv, venv) init), init = init, pos = pos } ) )
    |   fun transDec (venv, tenv, A.TypeDec lst) = let fun addTypeName ({ name, ty, pos }, env) = S.enter (env, name, T.NAME (name, ref NONE))
                                                       val tenv' = foldl addTypeName tenv lst
                                                       fun completeTypeName ({ name, ty, pos }, env) = S.enter (env, name, transTy (tenv', name))
                                                       val tenv'' = foldl completeTypeName tenv lst
                                                       fun validateTy ({ name, ty, pos }, cnt) = ( case ty of T.NAME (nm, tyref) => if nm = name andalso cnt = 0 then false
                                                                                                                                    else ( case !tyref of SOME t => validateTy ({ name, t, pos }, 0)
                                                                                                                                                             | _ => false )
                                                                                                            | _ => true )
                                                       fun checkCylicTyDec (x :: xs) = if validateTy (x, 1) then checkCylicTyDec xs else false
                                                   in if checkCylicTyDec lst then ( error pos "recursive type declaration not valid"; { venv = venv, tenv = tevn } )
                                                      else { venv = venv, tenv = tenv'' }
                                                   end

    and fun transTy (tenv, A.NameTy (sym, pos)) = ( case S.look (tenv, sym) of SOME t => t
                                                                             | _ => ( error pos "type '" ^ S.name sym ^ "' not defined"; T.UNIT ) )
    |   fun transTy (tenv, A.RecordTy lst) = let fun fieldTy { name, escape, typ, pos } = ( case S.look (tenv, name) of SOME t => (name, t)
                                                                                                                      | _ => ( error pos "type '" ^ S.name sym ^ "' not defined"; (name, T.UNIT) ) )
                                             in T.RECORD (map fieldTy lst, ref ()) end
    |   fun transTy (tenv, A.ArrayTy (sym, pos)) = ( case S.look (tenv, sym) of SOME t => T.ARRAY (t, ref ())
