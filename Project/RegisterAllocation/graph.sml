(* This file contains data structures and functions to implement graph for doing liveness analysis *)
...
signature GRAPH = sig
    ...
    structure Table : TABLE sharing type Table.key = node
...
structure Graph : GRAPH = struct
    ...
    fun succ (g, i) = let val NODE { succ = s, ... } = A.sub (g, i)
                      in map (augment g) s end
    fun pred (g, i) = let val NODE { pred = p, ... } = A.sub(g, i)
                      in map (augment g) p end
    fun adj (g, i) = pred (g, i) @ succ (g, i)

    fun newNode g = let fun look (lo, hi) = (* binary search for unused node, i < lo indicates i in use, i >= hi indicates i not in use *)
                                            if lo = hi then (A.update (g, lo, emptyNode); (g, lo))
                                            else let val m = (lo + hi) div 2
                                                 in if isBogus (A.sub (g, m)) then look (lo, m) else look (m + 1, hi) end
                    in look (0, 1 + A.bound g) end
    ...
    structure Table = IntMapTable ( type key = node fun getInt (g, n) = n )
    fun nodename (g, i) = "n" ^ (Int.toString i)
end